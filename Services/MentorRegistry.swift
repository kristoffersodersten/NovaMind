...
class MentorRegistry: ObservableObject {
    ...
    private func detectPerformanceDrift(pairingId: String) {
        ...
        let qualities = recentReflections.map { calculateReflectionQuality($0) }
        let averageQuality = qualities.reduce(0, +) / Double(recentReflections.count)
